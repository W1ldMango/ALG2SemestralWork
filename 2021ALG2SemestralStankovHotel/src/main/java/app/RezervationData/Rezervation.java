...
/**
 * The class responsible for the workability of the number registration
 */

public class Rezervation {

    private int roomNumberRezervation;
        ...
        if (lux.equals("yes")) {
            for (int i = 0; i < luxrooms.size(); i++) {
                if (places == luxrooms.get(i).getPlaces() && kids == luxrooms.get(i).getKids()) {
                    roomNumberRezervation = luxrooms.get(i).getNumber();
                    if (rezervationCheck()) {
            ...
            for (int i = 0; i < notluxrooms.size(); i++) {
                if (places == notluxrooms.get(i).getPlaces() && kids == notluxrooms.get(i).getKids()) {
                    roomNumberRezervation = notluxrooms.get(i).getNumber();
                    if (rezervationCheck()) {
     ...
     */
        ...
        BufferedImage img= ImageIO.read(new File("2021ALG2SemestralStankovHotel/lux.jpg"));
    ...
    private static void nonLuxImageRoom() throws IOException {
        BufferedImage img=ImageIO.read(new File("2021ALG2SemestralStankovHotel/nonlux.jpg"));
        ImageIcon icon=new ImageIcon(img);
        JFrame frame=new JFrame();
        frame.setLayout(new FlowLayout());
        frame.setSize(1024,681);
    ...
    public int lastPrice(int days) {
        ...
        for (int i = 0; i < rooms.size(); i++) {
            if (roomNumberRezervation == rooms.get(i).getNumber()) {
    ...
    private boolean rezervationCheck() throws IOException {
        roomsnumbers.clear();
        String trash;
        Scanner scanner = new Scanner(new File("2021ALG2SemestralStankovHotel/roomRezervation.txt"), StandardCharsets.UTF_8);
        try {
            do {
        ...
        for (int i = 0; i < roomsnumbers.size(); i++) {
            if (roomsnumbers.get(i).equals(Integer.toString(roomNumberRezervation))) {
                output = false;
                break;
            }
        } return output;
    ...
    public void writeRezervationEmail(String email) throws IOException {
        BufferedWriter writer = new BufferedWriter(new FileWriter("2021ALG2SemestralStankovHotel/roomRezervation.txt", true));
        writer.newLine();
        writer.append(email);
        writer.close();
    }
    ...
    protected void temp(int n) throws IOException {
        BufferedWriter writer = new BufferedWriter(new FileWriter("2021ALG2SemestralStankovHotel/temp.txt", false));
        //writer.newLine();
        writer.append("");
    ...
    public int rezervationRoom(){
        return this.roomNumberRezervation;
    }

    public void writeRezervationNumber(int n) throws IOException {
        Scanner scanner = new Scanner(new File("2021ALG2SemestralStankovHotel/temp.txt"));
        BufferedWriter writer = new BufferedWriter(new FileWriter("2021ALG2SemestralStankovHotel/roomRezervation.txt", true));
